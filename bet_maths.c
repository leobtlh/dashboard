...
function protocol_amount(current_protocol_fee, stake) {
    return stake * current_protocol_fee;
}
...
// === Valeur des tokens d’un rappeur spécifique ===
function tokens_value(total_liquidity, rapper_liquidity) {
    return rapper_liquidity / total_liquidity;
}
